...
export default function Home() {
  ...
  // 초기 로딩 - Hybrid 방식 (localStorage 캐시 + 서버 최신 데이터)
  useEffect(() => {
    if (!hydrated) return;
    ...
    const loadContent = async () => {
      try {
        ...
        if (cached) {
          ...
          console.log('✅ Initial content loaded from cache');
        }

        // 2️⃣ 서버에서 최신 데이터 가져오기 (백그라운드)
        console.log('📡 Fetching latest content from server...');
        ...
        if (response.ok) {
          ...
          if (Object.keys(serverData).length > 0) {
            ...
            console.log('✅ Content updated from server and cached');
          } else {
            console.log('⚠️ Server returned empty data');
            // 서버에 데이터 없고 캐시도 없으면 기본값
            if (!cached) {
          ...
          if (!cached) {
            const defaultData = fixHospitalContent({
              hero: {
                ...
                mainButton: '전화 연결하기',
                secondButton: '진료과목 보기',
                backgroundImage: 'Modern medical facility interior with warm orange lighting',
                backgroundImageFile: ''
              },
              services: { title: '주요 진료과목', subtitle: '전문적이고 체계적인 치료로 건강을 회복하세요', orthopedic: { title: '정형외과', description: '관절염, 골절, 척추질환 등 근골격계 질환의 전문적인 진단과 치료', items: ['관절염 치료', '척추질환 치료', '골절 치료', '스포츠 외상'], image: '', imageFile: '' }, anesthesia: { title: '마취통증의학과', description: '다양한 통증 질환의 정확한 진단과 효과적인 치료', items: ['만성 통증 치료', '신경차단술', '근막동통증후군', '대상포진 후 신경통'], image: '', imageFile: '' }, rehabilitation: { title: '재활의학과', description: '기능 회복과 삶의 질 향상을 위한 전문적인 재활치료', items: ['물리치료', '작업치료', '운동치료', '전기치료'], image: '', imageFile: '' } },
              doctors: { title: '의료진 소개', subtitle: '풍부한 경험과 전문성을 갖춘 의료진이 함께합니다', list: [{ name: '', position: '', career: [], image: '', imageFile: '' }, { name: '', position: '', career: [], image: '', imageFile: '' }, { name: '', position: '', career: [], image: '', imageFile: '' }] },
              facilities: { title: '시설 안내', subtitle: '최신 의료장비와 편리한 시설로 최상의 진료를 제공합니다', list: [{ name: '', description: '', image: '', imageFile: '' }, { name: '', description: '', image: '', imageFile: '' }, { name: '', description: '', image: '', imageFile: '' }, { name: '', description: '', image: '', imageFile: '' }] },
              contact: { title: '오시는 길', subtitle: '편리한 교통과 주차시설을 제공합니다', address: '부산 동구 중앙대로 375 (수정동) 강남빌딩 3층', phone: '051-469-7581', hours: { weekday: '평일: 09:00 - 18:00', saturday: '토요일: 09:00 - 13:00', sunday: '일요일 및 공휴일: 휴진' }, parking: '건물 지하 주차장 이용 가능', subway: '2호선 해운대역 3번 출구 도보 5분', bus: '해운대역 정류장 하차 후 도보 3분' },
              footer: { companyName: '비타민마취통증의학과의원', address: '부산 동구 중앙대로 375 (수정동) 강남빌딩 3층', phone: '051-469-7581', email: 'info@vitamin-clinic.co.kr', businessNumber: '123-45-67890', representative: '김철수', copyright: '© 2024 비타민마취통증의학과의원. All rights reserved.', links: { privacy: '/privacy', terms: '/terms', sitemap: '/sitemap' } }
            });
            setContentData(defaultData);
            console.log('✅ Default data loaded (server failed)');
          }
        }
      } catch (e) {
