    ...
    <section id="services" className="py-24 bg-gradient-to-br from-gray-50 to-orange-50/20">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 mb-4">{servicesData.sectionTitle || '주요 진료 서비스'}</h2>
          <p className="text-base sm:text-lg text-gray-700 max-w-2xl mx-auto leading-relaxed">
            {servicesData.sectionDescription || '환자 맞춤형 진료 서비스로 통증 없는 일상을 만들어드립니다.'}
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {servicesData.services.map((service, index) => (
              ...
              >
                {/* Image */}
                <div className="relative aspect-video bg-gradient-to-br from-white via-orange-50 to-orange-100 overflow-hidden">
                  {service.image ? (
                    <Image src={service.image} alt={service.title} fill className="object-contain group-hover:scale-105 transition-transform duration-500" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center">
                      <i className={`${service.icon} text-6xl sm:text-7xl text-orange-400`}></i>
                    </div>
                  )}
                ...
                <div className="p-6">
                  <div className="flex items-center mb-3">
                    <div className="w-12 h-12 bg-orange-100 rounded-xl flex items-center justify-center mr-4 flex-shrink-0 group-hover:bg-orange-500 group-hover:scale-110 transition-all duration-300">
